...
之所以选择用油猴脚本而不是用浏览器扩展来实现，是因为油猴脚本一次编写多平台通用，并且无需经过签名、编译等流程即可发布，开发成本和门槛更低，但代价是油猴脚本在浏览器中的权限层级也更低导致无法实现某些功能，后续可能会重新实现为浏览器扩展。
...
|🧩 Feature|输入框划词-新增换行按钮 | 当用户点击输入框中任意一行的末尾时，根据光标位置显示换行按钮，点击即可在此处换行|待定|
|🧩 Feature|输入框划词-新增清空按钮 | 当用户选中输入框且输入框中存在文本时，根据输入框位置显示清空按钮，点击即可清空输入框中的文本|待定|
